class Input
    ...
    def initialize(input)
        @input = input.strip
        parsed_inputs = @input.split(' -> ').map{|c| c.split(',')}
        
        @x1 = parsed_inputs[0][0].to_i
        @x2 = parsed_inputs[1][0].to_i
        @y1 = parsed_inputs[0][1].to_i
        @y2 = parsed_inputs[1][1].to_i
    end
...
lines = File.readlines('input').map do |line|
  ...
  answer = world.vents.values.filter {|v| v > 1}.length
end
